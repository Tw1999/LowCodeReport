...
# 调试参数处理 - 支持多种格式
debug_param = params.get("debug", False)
if isinstance(debug_param, str):
    debug = debug_param.strip().lower() in ('true', '1', 'yes')
else:
    debug = bool(debug_param)

if not fee_date_start or not fee_date_end:
    set_result(rows=[], message="费用开始时间和结束时间为必填参数")
else:
    # 处理参数
    if comm_id:
    ...
    # 生成调试SQL的函数
    def generate_debug_sql(sql, args):
        """生成调试SQL，将参数替换到SQL中"""
        debug_sql = sql
        for arg in args:
            if arg is None:
...
'''
    ...
    year_args.append(fee_date_end_with_time)
...
'''
    year_args.append(fee_date_start)
    year_args.append(fee_date_end_with_time)
    year_args.append(receipt_end_date_with_time)
...
'''
    ...
    debug_sqls = {}
    if debug:
        debug_year_sql = generate_debug_sql(year_sql, year_args)
        debug_sqls['year_query_sql'] = debug_year_sql
    ...
    if not years:
        if debug:
            set_result(rows=[{"debug_info": "年份查询无数据", "year_query_sql": debug_sqls.get('year_query_sql', '')}], message="调试模式-年份查询无数据")
        else:
        ...
        year_case_list = []
        for year in years:
            year_case_list.append(f"SUM(CASE WHEN year(fee_date) = {year} THEN 欠费金额含税金额 ELSE 0 END) AS `{year}年`")
        ...
        args.append(fee_date_end_with_time)
        ...
        args.append(fee_date_start)
        args.append(fee_date_end_with_time)
        args.append(receipt_end_date_with_time)
...
'''
        ...
        # 如果开启调试模式，返回所有SQL
        if debug:
            debug_main_sql = generate_debug_sql(sql, args)
            debug_sqls['main_query_sql'] = debug_main_sql
            set_result(
                rows=[{
                    "year_query_sql": debug_sqls.get('year_query_sql', ''),
                    "main_query_sql": debug_main_sql,
                    "years_found": years
                }],
                message="调试模式-返回所有SQL"
            )
        else:
            # 执行查询
