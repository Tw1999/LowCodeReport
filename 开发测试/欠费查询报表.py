...
def get_fee_statistics_by_criteria(params: Dict[str, Any]) -> Dict[str, Any]:
    # 参数解析
...
def build_year_query(
    comm_id: Optional[str],
    delivery_status: str,
    cost_id_list: List[str],
    fee_date_start: str,
    fee_date_end: str,
    receipt_end_date: str
) -> Tuple[str, List[Any]]:
    """
    构建年份查询SQL
    """
    ...
    # 处理delivery_status条件，如果为"0"则不过滤
    if delivery_status != "":
...
def build_main_query_sql(
    ...
    # 处理delivery_status条件，如果为"0"则不过滤
    if delivery_status != "" and delivery_status is not None:
