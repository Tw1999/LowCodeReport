...
debug_mode = params.get("debug_mode", "").strip().lower() == "true"  # 是否开启调试模式，默认为false
...
# 标准化日期格式
if fee_start_date:
    fee_start_date = normalize_date(fee_start_date, "00:00:00")
if fee_end_date:
    fee_end_date = normalize_date(fee_end_date, "23:59:59")
if deal_date_start:
    deal_date_start = normalize_date(deal_date_start, "00:00:00")
if deal_date_end:
    deal_date_end = normalize_date(deal_date_end, "23:59:59")

# 验证时间范围：费用结束时间不能小于费用开始时间
error_message = ""
...
if error_message:
    ...
    def should_add_date_condition(date_value, boundary_value):
        ...
        return normalized_date != boundary_value
...
END AS resource_status,
        ...
        b.bill_sign,
        CASE 
            WHEN d.is_delete = '1' THEN '直接撤销'
            WHEN d.is_delete = '0' AND d.deal_type = '减免红冲' THEN '红冲撤销'
            ELSE ''
        END AS cancel_type
    FROM tb_charge_receipts_detail d
    LEFT JOIN erp_base.rf_organize o ON d.comm_id = o.Id AND o.OrganType = 6 AND o.Is_Delete = 0 AND o.Status = 0
    LEFT JOIN erp_base.rf_organize o1 ON o.ParentId = o1.Id AND o1.Is_Delete = 0 AND o1.Status = 0
    LEFT JOIN erp_base.tb_base_masterdata_resource r ON d.resource_id = r.id
    LEFT JOIN erp_base.tb_base_charge_cost c ON d.corp_cost_id = c.id AND c.is_delete = 0
    LEFT JOIN erp_base.tb_base_charge_comm_stan st ON d.stan_id = st.id AND st.is_delete = 0
    ...
    LEFT JOIN erp_base.tb_base_masterdata_customer_comm cc ON d.customer_id = cc.id AND cc.is_delete = 0
    WHERE 1=1
    """
    ...
    if is_cancel == '0':
        # 未撤销：只查询d.deal_type='减免'且is_delete=0
        ...
        if cancel_type == '直接撤销':
            ...
            # 红冲撤销：查询d.deal_type='减免红冲'（is_delete默认为0）
            sql += " AND d.deal_type = '减免红冲'"
        else:
            # cancel_type为空时，查询所有撤销类型
            # 注意：减免红冲记录的is_delete通常为0，而直接撤销的减免记录is_delete为1
            sql += " AND ( (d.deal_type = '减免红冲' OR d.deal_type = '减免' ) AND d.is_delete = 1 )"
    else:
        # 如果is_cancel为空或无效，默认按未撤销处理
        sql += " AND d.deal_type = '减免'"
    ...
    if comm_ids:
        ...
        if comm_ids:
            ...
            sql += f" AND d.comm_id IN ({placeholders})"
    ...
    if corp_cost_ids:
        ...
        if corp_cost_ids:
            ...
            sql += f" AND d.corp_cost_id IN ({placeholders})"
            args.extend(corp_cost_ids)

    # 处理费用开始时间参数
    # 只有当传入的不是最小边界值时才添加条件
    if should_add_date_condition(fee_start_date, MIN_DATE):
        sql += " AND d.fee_start_date >= %s"
        args.append(fee_start_date)

    # 处理费用结束时间参数
    # 只有当传入的不是最大边界值时才添加条件
    if should_add_date_condition(fee_end_date, MAX_DATE):
    ...
    sql += " ORDER BY d.is_delete DESC LIMIT %s OFFSET %s"
    ...
    try:
        ...
        # 根据调试模式返回不同结果
        if debug_mode:
            # 调试模式下将SQL、参数和行数添加到message中返回
            debug_message = f"查询成功（调试模式）\nSQL: {sql}\nParams: {args}\n返回行数: {row_count}"
            set_result(rows=dataRows, message=debug_message)
        else:
            # 正常模式下返回结果和行数
            set_result(rows=dataRows, message=f"查询成功，返回行数: {row_count}")
