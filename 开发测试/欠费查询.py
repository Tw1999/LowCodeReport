...
comm_ids = "0591cfbd-d915-48d6-9831-7f6201cd4d3e,0d8400d6-d728-4f72-9e6c-08bb9bf5828d".split(",") ;
...
LEFT JOIN (
    ...
    FROM tidb_sync_combine.tb_charge_receipts_detail f  -- 别名改为rd，避免与外层f冲突
    WHERE 1=1
        AND deal_type IN ('实收', '代扣', '托收确认', '实收红冲', '预存冲抵', '预存冲抵红冲', '减免', '减免红冲')
        AND is_delete = 0
        {comm_ids_condition}
        {corp_cost_ids_condition}
        {resource_status_condition}
        AND f.fee_date BETWEEN %s AND %s  -- 子查询的日期参数（单独占位符）
        AND f.deal_date <= %s  -- 子查询的截止日期参数
    GROUP BY fee_id
...
try:
    ...
    for i in range(year_count):
        ...
        print(f"年度：{debtsyear}")  # 现在能正常输出2025、2024等
